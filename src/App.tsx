  ...
  return (
    <Link
      ...
      className={`block sm:inline rounded-none sm:rounded-md px-3 py-2 text-base sm:text-sm font-medium transition ${
        ...
        !loading ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-full"
      } ${
        !displayed || loading
              ...
              <div className="relative flex h-16 items-center justify-between">
                ...
                <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                  <div className="flex shrink-0 items-center">
                    <img className="size-8 rounded-full" src={me} alt="yo" />
                  </div>
                  <div className="hidden sm:ml-6 sm:block">
                    <div className="flex space-x-4">
                      {routes.map(
                        ({ name, link, hidde }, key) =>
                          !hidde && (
                            <NavLink key={key} href={urlBase + link}>
                              {t(`links.${name}`)}
                            </NavLink>
                          ),
                      )}
                      ...
                      <div onClick={() => setShowMenu(false)}>
                        ...
                        >
                          <div className="flex items-center justify-between ">
                            ...
                            <div className="">{t(`links.${name}`)}</div>
                          </div>
                        </NavLink>
                      </div>
                    ),
        ...
        <main className="mx-0 p-1 md:p-4 pb-10 flex-grow h-full">
          <Switch>
            {routes.map(({ component, link }, key) => (
              <Route key={key} path={urlBase + link} component={component} />
            ))}
            <Route>
              <Redirect to={urlBase} />
            </Route>
